...
async function agregarALista(id) {
    if (productosEnCola[id]) {
        productosEnCola[id].cantidadAñadir += 1;
        actualizarInterfaz();
        return;
    }
        ...
        cuerpo.innerHTML += `
            <tr>
                ...
                <td style="text-align: center;">
                    <input type="number" class="input-cantidad-lista" value="${p.cantidadAñadir}" 
                    onchange="window.cambiarCantidadManual('${id}', this.value)">
                </td>
                <td>
                    <button onclick="window.eliminarDeLista('${id}')" class="btn-eliminar-item">&times;</button>
                </td>
            </tr>`;
    });
}

// --- FLUJO DE VALIDACIÓN (ENTREGA Y AUTORIZA) ---
window.procesarEntradaCompleta = function() {
    ...
    try {
        for (const id of ids) {
            ...
            await updateDoc(doc(db, "articulos", id), {
                cantidad: increment(p.cantidadAñadir),
            ...
            await addDoc(collection(db, "entradas_historial"), {
                productoId: id,
                nombre: p.nombre,
                cantidadIngresada: p.cantidadAñadir,
...
window.cambiarCantidadManual = (id, valor) => { if(productosEnCola[id]) productosEnCola[id].cantidadAñadir = parseInt(valor) || 1; };
